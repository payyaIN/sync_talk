...
usersRouter.get('/me', requireAuth, async (req: AuthedRequest, res) => {
  ...
  res.json({ id: (me._id as Types.ObjectId).toString(), email: me.email, displayName: me.displayName, role: me.role, avatarUrl: me.avatarUrl });
});
usersRouter.get('/search', requireAuth, async (req, res) => {
  ...
  res.json(users.map(u => ({ id: (u._id as Types.ObjectId).toString(), displayName: u.displayName, email: u.email, avatarUrl: u.avatarUrl })));
});
usersRouter.get('/', requireAuth, requireAdmin, async (_req, res) => {
  const users = await User.find().sort({ createdAt: -1 }).limit(200);
  res.json(users.map(u => ({ id: (u._id as Types.ObjectId).toString(), email: u.email, displayName: u.displayName, role: u.role, banned: u.banned })));
});
